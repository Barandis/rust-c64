...
impl Ic74257 {
    /// Creates a new 74257 quad 2-to-1 multiplexer emulation and returns a shared,
...
mod test {
    ...
    fn mux_1_select_a() {
        ...
        assert!(
            low!(tr[Y1]),
            "Y1 should be low when A1 is low and SEL is low"
        );

        set!(tr[A1]);
        assert!(
            high!(tr[Y1]),
            "Y1 should be high when A1 is high and SEL is low"
        )
    }
    ...
    fn mux_1_select_b() {
        ...
        assert!(
            high!(tr[Y1]),
            "Y1 should be high when B1 is high and SEL is high"
        );

        clear!(tr[B1]);
        assert!(
            low!(tr[Y1]),
            "Y1 should be low when B1 is low and SEL is high"
        );
    }
    ...
    fn mux_2_select_a() {
        ...
        assert!(
            low!(tr[Y2]),
            "Y2 should be low when A2 is low and SEL is low"
        );

        set!(tr[A2]);
        assert!(
            high!(tr[Y2]),
            "Y2 should be high when A2 is high and SEL is low"
        )
    }
    ...
    fn mux_2_select_b() {
        ...
        assert!(
            high!(tr[Y2]),
            "Y2 should be high when B2 is high and SEL is high"
        );

        clear!(tr[B2]);
        assert!(
            low!(tr[Y2]),
            "Y2 should be low when B2 is low and SEL is high"
        );
    }
    ...
    fn mux_3_select_a() {
        ...
        assert!(
            low!(tr[Y3]),
            "Y3 should be low when A3 is low and SEL is low"
        );

        set!(tr[A3]);
        assert!(
            high!(tr[Y3]),
            "Y3 should be high when A3 is high and SEL is low"
        )
    }
    ...
    fn mux_3_select_b() {
        ...
        assert!(
            high!(tr[Y3]),
            "Y3 should be high when B3 is high and SEL is high"
        );

        clear!(tr[B3]);
        assert!(
            low!(tr[Y3]),
            "Y3 should be low when B3 is low and SEL is high"
        );
    }
    ...
    fn mux_4_select_a() {
        ...
        assert!(
            low!(tr[Y4]),
            "Y4 should be low when A4 is low and SEL is low"
        );

        set!(tr[A4]);
        assert!(
            high!(tr[Y4]),
            "Y4 should be high when A4 is high and SEL is low"
        )
    }
    ...
    fn mux_4_select_b() {
        ...
        assert!(
            high!(tr[Y4]),
            "Y4 should be high when B4 is high and SEL is high"
        );

        clear!(tr[B4]);
        assert!(
            low!(tr[Y4]),
            "Y4 should be low when B4 is low and SEL is high"
        );
    }
